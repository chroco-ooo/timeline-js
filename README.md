...
- 横型グリッドの年またぎを通常罫線と同系色の実線と淡い余白で表示
...
- 縦型の現在位置を「NOW」で表示（表示期間内の場合のみ）
- 画像付きイベントのサムネイルをカード上部へ全幅表示
- 縦型カードでは画像、日付と見出し、説明、名前と詳細アクションを順に表示
...
- 縦型のNOWはシステム日時が `startDate` と `endDate` の範囲内の場合だけ表示します。基準日時を指定する公開オプションはありません。
- 画像付き縦型カードは画面幅にかかわらずサムネイル、本文の順に縦積みします。長いタイトルと説明は省略せず折り返します。
